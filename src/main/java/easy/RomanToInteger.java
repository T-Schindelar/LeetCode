 ...
 * <br><br>
 * Example 3: Input: s = "MCMXCIV" Output: 1994 Explanation: M = 1000, CM = 900,
 * XC = 90 and IV = 4.
 * <br><br>
 * Constraints: 1 <= s.length <= 15 s contains only the characters ('I', 'V',
 * 'X', 'L', 'C', 'D', 'M'). It is guaranteed that s is a valid roman numeral in
 * the range [1, 3999].
 */
public class RomanToInteger {
    ...
    public static int romanToInt(String s) {
        ...
        for (int i = s.length() - 1; i >= 0; i--) {
            if (i - 1 >= 0 && MAP.containsKey(s.substring(i - 1, i + 1))) {
